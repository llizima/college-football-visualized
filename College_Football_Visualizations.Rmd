...
# Define top conferences
top_confs <- c(
  "SEC", "Big Ten", "ACC", "Pac-12", "Big 12",
  "American Athletic", "Mountain West", "Conference USA", "FBS Independents"
)

# Step 1: Clean + standardize + group conferences
df_rank_vs_conf <- sp_data %>%
  ...
  mutate(
    ...
    # Collapse into top conferences or "Other"
    conference = ifelse(conference %in% top_confs, conference, "Other"),

    # Define ranking group
    ...
    # Factor levels for ordering x-axis
    ranking_group = factor(ranking_group, levels = c("1-5", "6-10", "11-15", "Others"))
  )

# Step 2: Group and summarize
conf_vs_rank <- df_rank_vs_conf %>%
  count(ranking_group, conference, name = "count") %>%
  group_by(ranking_group) %>%
  mutate(
    Percent = 100 * count / sum(count),
    conference = fct_reorder2(conference, ranking_group, -Percent)  # stack largest % at bottom
  ) %>%
  ungroup()

# Step 3: Define consistent color map
color_map <- c(
  "SEC" = "#E69F00",
  "Big Ten" = "#56B4E9",
  "ACC" = "#009E73",
...
# Step 4: Plot
p <- ggplot(conf_vs_rank, aes(x = ranking_group, y = Percent, fill = conference)) +
  ...
  labs(
    x = "Ranking Group",
    y = "Percent (%)",
    title = "Top 15 Ranking Distribution by Conference",
    subtitle = "All other teams grouped as 'Other'"
  ) +
  theme_minimal() +
  theme(
...
p_plotly <- plot_ly(
  ...
  layout(
    title = list(
      text = "<b>Defense Rating vs Ranking</b><br><span style='font-size:12pt;'>2024–2025 Season</span>",
      x = 0.5,
      xanchor = "center"
    ),
    margin = list(t = 80),
